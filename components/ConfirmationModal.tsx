...
const ConfirmationModal: React.FC<ConfirmationModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[200] flex items-center justify-center p-4 animate-in fade-in duration-200 backdrop-blur-sm">
      <div className="bg-white rounded-[2rem] w-full max-w-md shadow-2xl overflow-hidden transform transition-all scale-100 border border-white/20">
        ...
        <div className={`px-6 py-4 flex items-center gap-3 ${isDangerous ? 'bg-red-50 border-b border-red-100' : 'bg-gray-50 border-b border-gray-100'}`}>
          ...
          <button onClick={onClose} className="ml-auto text-gray-400 hover:text-gray-600 p-1 hover:bg-white/50 rounded-full transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="bg-gray-50 px-6 py-5 flex justify-end gap-3 border-t border-gray-100">
          <button
            onClick={onClose}
            className="px-6 py-3 bg-white border border-gray-300 rounded-xl text-gray-700 font-bold text-[10px] uppercase tracking-widest hover:bg-gray-50 transition-colors shadow-sm"
          >
            {cancelText}
          </button>
          <button
            ...
            className={`px-8 py-3 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest shadow-lg transition-all active:scale-95 ${
              isDangerous 
